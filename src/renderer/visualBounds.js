...
export function isVisuallyVisible(element) {
  // A deferred update scope has accepted the logical visibility attribute but has not projected
  // it to the rendered 2D family yet. Animation scheduling can still validate that logical state.
  if (element.is3D && element.board.isUpdateDeferred())
