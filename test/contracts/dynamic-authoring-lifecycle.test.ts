...
describe("Dynamic authoring lifecycle contracts", () => {
    ...
    it("拒绝非法书写时长且不损坏原文本", () => {
        const currentBoard = createBoard();
        const text = currentBoard.create("text", [0, 0, "recoverable"], { visible: false });
        expect(() => text.write(Number.NaN)).toThrowError(/duration/);
        expect(text.plaintext).toBe("recoverable");
        expect(text.getAttribute("visible")).toBe(false);
        text.write(0);
        expect(text.getAttribute("visible")).toBe(true);
    });
